...
import subprocess
import pdb
root=repo_dir = subprocess.Popen(['git','rev-parse','--show-toplevel'], stdout=subprocess.PIPE).communicate()[0].rstrip()
...
class Xomo:
  ...
  def __init__(
          self,
          out='/home/rkrish11/git/Experimental-Algorithms/multiProc/src',
          # root+"multProc/src/Models/xomo/",
          data="data",
          model=None):
    def theModel(model):
...
def howMuchEffort(row):
  X = Xomo(model="all")
  return X.c.xys(x=row)


def pom3d(N=50):
  p = Pom()
  return p.trials(N, verbose=False)
