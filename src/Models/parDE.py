...
def de0(model=DTLZ2(n_dec=30,n_obj=3), new=[], pop=int(1e4), iter=1000, lives=50, settings=settings):
  """
  ...
  while iter>0:
    ...
    try: xbestVal = model.solve(xbest)
    except: set_trace()
...
def de1(iter=1000,pop=100,n_proc=2,model=DTLZ2(n_dec=30, n_obj=3)):
