...
[1] Quinlan, J. Ross. "Improved use of continuous attributes in C4. 5." Journal
 of artificial intelligence research (1996): 77-90.
[2] Quinlan, J. Ross. C4. 5: programs for machine learning. Elsevier, 2014.

Also uses,
[3] Usama M. Fayyad, Keki B. Irani. "Multi-interval discretization of continuous
valued attributes for classification learning." Thirteenth International Joint
...
def encode(tbl, features, opt=None):
    new=[]
    if opt==None:
        maxLen = 10
    else:
...
def dtree2(tbl, rows=None, lvl=-1, asIs=10 ** 32, up=None, klass=-5, branch=[],
           f=None, val=None, opt=None, encode=True):
    """
    ...
    ent = lambda x: sum([-Counter[n] / len(x) * np.log(Counter[n] / len(x)) for n in Counter.keys()])
    ...
    for child, span in rows():
        ...
        if opt.min <= n < N:
            here.kids += [dtree2(child, lvl=lvl + 1, asIs=toBe, up=here
                                 , branch=branch + [(name, span)]
                                 , f=name, val=(span, span), opt=opt, encode=False)]
...
def _test():
    tbl_loc = explore(directory='../Data/Seigmund/', name='Apache')
    tbl = csv2df(tbl_loc)
