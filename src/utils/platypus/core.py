...
class Problem(object):
    ...
    def __init__(self, nvars, nobjs, nconstrs = 0, function=None, indep=None, depen=None, name=None):
        super(Problem, self).__init__()
        self.nvars = nvars
        self.nobjs = nobjs
        self.name = "noname" if name is None else name
        self.nconstrs = nconstrs
        self.function = function
        self.types = FixedLengthArray(nvars)
        self.indep = nvars*["Untitled"] if indep is None or not isinstance(indep, list) else indep
        self.depen = nobjs*["Untitled"] if depen is None or not isinstance(depen, list) else depen
...
class MaxEvaluations(TerminationCondition):
    ...
    def shouldTerminate(self, algorithm):
        self.starting_nfe += 1
        return self.starting_nfe >= self.nfe
...
class Algorithm(object):
    ...
    def __init__(self,
        ...
        self.log_frequency = skip
        self.nfe = 0
        self.logging = saveLogs
    ...
    def run(self, condition):
        ...
        with open(self.log_file, 'w') as csvfile:
            my_CSV = csv.writer(csvfile)
            my_CSV.writerow(self.problem.indep+self.problem.depen)
            while not condition(self):
                self.step()
                if self.logging:
                    for row in self.population:
                        my_CSV.writerow(row.asList())
...
class Solution(object):
    ...
    def __str__(self):
        return "[" + ",".join(list(map(str, self.variables))) + "|" + ",".join(list(map(str, self.objectives)))+"]"

    def asList(self):
        res = self.__str__().strip("[").strip("]").split('|')
        var = [float(a) for a in res[0].split(',')]
        obj = [float(a) for a in res[1].split(',')]
        return var+obj

    def values(self):
        from re import sub
        myString = sub('[\[\]]', '', self.__str__())
        attr = myString.split('|')[0].split(',')
        attr = [float(j) for j in attr]
        # set_trace()
        return attr
    ...
    def obj(self):
        from re import sub
        myString = sub('[\[\]]', '', self.__str__())
        obj = myString.split('|')[1].split(',')
        obj = [float(aa) for aa in obj]
        return obj
